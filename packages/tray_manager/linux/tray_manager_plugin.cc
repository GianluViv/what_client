...
static FlMethodResponse* set_icon(TrayManagerPlugin* self, FlValue* args) {
  ...
  // AppIndicator expects an icon NAME (looked up in the icon theme), not a
  // file path.  When tray_manager passes an absolute path we split it into:
  //   • icon_theme_path  — the parent directory (registered as a theme search path)
  //   • icon_name        — the basename without extension
  //
  // The icon must be placed at:
  //   <icon_theme_path>/hicolor/<size>x<size>/apps/<icon_name>.png
  // following the Freedesktop icon theme spec so that GTK's icon-theme engine
  // can locate it via the search path.
  gchar* icon_dir = g_path_get_dirname(icon_path);
  gchar* basename = g_path_get_basename(icon_path);
  gchar* dot = g_strrstr(basename, ".");
  gchar* icon_name = dot ? g_strndup(basename, dot - basename) : g_strdup(basename);
  g_free(basename);

  if (!indicator) {
    indicator = app_indicator_new(id, icon_name,
  ...
  app_indicator_set_icon_full(indicator, icon_name, "");
