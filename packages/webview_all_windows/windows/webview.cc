...
// Shows the native "Save As" dialog for a download, pre-filled with
// WebView2's suggested destination path. Returns false if the user cancels.
bool ShowDownloadSaveDialog(HWND owner, const std::wstring& suggested_path,
                            std::wstring* out_path) {
  wchar_t file_buffer[MAX_PATH] = {};
  if (!suggested_path.empty()) {
    wcsncpy_s(file_buffer, suggested_path.c_str(), _TRUNCATE);
  }
    ...
    webview24->add_DownloadStarting(
        Callback<ICoreWebView2DownloadStartingEventHandler>(
            [this](ICoreWebView2* sender,
              ...
              const bool proceed = ShowDownloadSaveDialog(
                  hwnd_,
                  resultFilePath.get() != nullptr ? resultFilePath.get()
                                                  : L"",
                  &chosenPath);
              ...
              args->put_ResultFilePath(chosenPath.c_str());
              ...
              if (download_event_callback_) {
                download_event_callback_(
                    {WebviewDownloadEventKind::DownloadStarted,
                     ...
                     util::Utf8FromUtf16(chosenPath.c_str()), 0,
                     totalBytesToReceive});
              }
